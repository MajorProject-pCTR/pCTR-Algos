logistic_join_sub <- function() {

	library(biglm)
	start <- Sys.time()
	ad_data <- read.table("unrolled_processed_sub.txt")
	end <- Sys.time()
	diff <- end - start
	...
	start <- Sys.time()
	glm <- bigglm(if_click~depth+position+ad_url_freq+ad_id_freq+adv_id_freq+user_id_freq+gender+age+desc_id_freq+desc_id_tokencnt+t_id_freq+t_id_tokencnt+k_id_freq+k_id_tokencnt+qid_freq+qid_tokencnt, data=ad_data, family=binomial())
	print(summary(glm))
	end <- Sys.time()
